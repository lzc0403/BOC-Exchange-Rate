...
中国银行外汇牌价历史抓取 - v5 生产版
全面处理边界情况:
  1. 翻页失败 → 重试(递增等待) → 重新取验证码
  2. 网络异常 → 捕获并重试
  3. Session 过期 → 每 100 天或出错时自动重建
  4. 服务端错误页(系统繁忙等) → 快速识别跳过等待
  5. 进度追踪 → 每 10 天打印一次进度
"""
import re
import time
...
MAX_DAY_ATTEMPTS = 8      # 单日最大重试次数(含验证码失败、翻页失败)
PAGE_RETRY       = 3      # 单页最大重试次数
SESSION_REFRESH  = 100    # 每处理 N 天自动重建 session(防 JSESSIONID 过期)
...
def make_session() -> requests.Session:
    ...
    try:
        ...
        log.warning(f"Session 初始化警告(不影响运行): {e}")
    return s
...
def crossed_target(rows: list[dict], d: date) -> bool:
    """当前页最后一条是否已早于 10:00 → 目标数据已全部收集完毕"""
    if not rows:
        return False
...
def fetch_page_with_retry(
    ...
    for attempt in range(1, PAGE_RETRY + 1):
        ...
        if has_server_error(html) and not parse_table(html):
            log.warning(f"    第{page_no}页第{attempt}次服务端错误提示, 等待重试")
        ...
        if rows:
            ...
            if new_pf:
                old_tk = pf.get("paramtk", "")[:12]
                new_tk = new_pf.get("paramtk", "")[:12]
                if old_tk != new_tk:
                    log.info(f"      paramtk 已刷新")
                pf = new_pf
            else:
                log.warning(f"      第{page_no}页响应无 pageform, 沿用旧 paramtk")
            return rows, pf
...
def fetch_one_day(session, d: date) -> dict | None:
    ...
    for day_attempt in range(1, MAX_DAY_ATTEMPTS + 1):
        ...
        if not (3 <= len(captcha) <= 6):
            log.info(f"  #{day_attempt} OCR={captcha!r} 长度异常")
            continue
        ...
        if has_server_error(html1) and not parse_table(html1):
            log.info(f"  #{day_attempt} 验证码/服务端错误")
            continue
        ...
        if not crossed_target(rows1, d):
            for page_no in range(2, page_count + 1):
                ...
                if not rows_p:
                    log.warning(f"  第{page_no}页重试全部失败 → 重取验证码重启")
                    day_ok = False
                    break
        ...
        cands = [(t, r) for t, r in cands
                 if t and t.date() == d and t.hour >= TARGET_HOUR]

        if not cands:
            log.warning(f"  {d} 10:00 后无数据(节假日/停牌)")
            return None
    ...
    log.error(f"  ✗ {d} 已重试 {MAX_DAY_ATTEMPTS} 次, 跳过")
    return None
...
def main():
    ...
    log.info(f"总范围: {START_DATE} → {END_DATE} | 已抓: {len(done)} 天 | 待抓: {len(pending)} 天")
    ...
    for d in pending:
        ...
        # 定期刷新 session(防 JSESSIONID 过期)
        if processed > 0 and processed % SESSION_REFRESH == 0:
            log.info(f"--- 定期刷新 session (已处理 {processed} 天) ---")
            session = make_session()
        ...
        if processed % 10 == 0:
            ...
            log.info(f">>> 进度: {processed}/{len(pending)} ({pct:.1f}%) <<<")

        time.sleep(random.uniform(0.8, 1.5))

    log.info(f"== 全部完成, 共处理 {processed} 天 ==")
